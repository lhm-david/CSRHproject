...
export default function ReportPage() {
  ...
  useEffect(() => {
    const numAlcoholSales = parseFloat(alcoholSales);
    const numNetSales = parseFloat(netSales);
    const numTotalGuest = parseFloat(totalGuest);
    const numCreditCardTips = parseFloat(creditCardTips);
    const numCashTips = parseFloat(cashTips);
    const numTotayTotalScan = parseFloat(totayTotalScan);
    const numTotalTable = parseFloat(totalTable);


    setAlcoholSalesPercentage(
        (!isNaN(numAlcoholSales) && !isNaN(numNetSales) && numNetSales !== 0) ? (numAlcoholSales / numNetSales * 100).toFixed(2) : "0.00"
    );

    setAlcoholSalesPerGuest(
        (!isNaN(numAlcoholSales) && !isNaN(numTotalGuest) && numTotalGuest !== 0) ? (numAlcoholSales / numTotalGuest).toFixed(2) : "0.00"
    );

    const calculatedTotalTips = (!isNaN(numCreditCardTips) && !isNaN(numCashTips)) ? (numCreditCardTips + numCashTips) : 0;
    setTotalTips(calculatedTotalTips.toFixed(2));


    setTipsPercentage(
        (!isNaN(calculatedTotalTips) && !isNaN(numNetSales) && numNetSales !== 0) ? (calculatedTotalTips / numNetSales * 100).toFixed(2) : "0.00"
    );

    setSalesPerGuest(
        (!isNaN(numNetSales) && !isNaN(numTotalGuest) && numTotalGuest !== 0) ? (numNetSales / numTotalGuest).toFixed(2) : "0.00"
    );

    setScanRate(
        (!isNaN(numTotayTotalScan) && !isNaN(numTotalTable) && numTotalTable !== 0) ? (numTotayTotalScan / numTotalTable * 100).toFixed(2) : "0.00"
    );

  }, [alcoholSales, netSales, totalGuest, creditCardTips, cashTips, totayTotalScan, totalTable]);
  ...
  const handleSaveLocal = async () => {
    ...
    try {
        ...
        // Save to server's public/reportFiles folder
        const serverResult = await saveReportToServer(report, filename);
